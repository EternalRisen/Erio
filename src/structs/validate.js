...
exports.checkProperties = exports.checkCommandModule = void 0;
function checkCommandModule(cmdName, cmdModule) {
    ...
    if (!cmdModule.hasOwnProperty('type')) {
        throw new Error(`${cmdName} command module does not have property 'type'.`);
    }
...
exports.checkCommandModule = checkCommandModule;
;
function checkProperties(cmdName, cmdModule) {
    ...
    if (typeof cmdModule.type !== 'string') {
        throw new Error(`${cmdName} command: 'type' is not a string`);
    }
    if (!typeTypes.includes(cmdModule.type)) {
        throw new Error(`${cmdName} command:  'type' should be ${typeTypes} which doesn't include '${cmdModule.type}'`);
    }
